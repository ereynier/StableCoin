 ...
 * This stablecoin has theproperties:
 *  - Exogenous collateral
 *  - Algorithmic Stable
 *  - Dollar Pegged
 *
 * It is similar to the DAI stablecoin, but with no governance, no fees, and only backed by wETH and wBTC.
 *
 * Our DSC should always be "over-collateralized" by at least 200%.
 *
...
contract DSCEngine is ReentrancyGuard {
     ...
     * @param amountDscToMint The amount of Decentralized StableCcoin to mint
     ...
     * @param amountDscToBurn The amount of Decentralized StableCcoin to burn
     ...
     * @notice You will receive the collateral at a discount
     * @notice This function working assumes the protocol will be roughly 200% overcollateralized in order to work
     * @notice A known bug would be if the price of the collateral token drops significantly, the protocol could be undercollateralized
     */
        ...
        nonReentrant
    {
        ...
        if (startingUserHealthFactor >= MIN_HEALTH_FACTOR) {
            revert DSCEngine__HealthFactoOk(startingUserHealthFactor);
        }
        uint256 tokenAmountFromDebtCovered = getTokenAmountFromUsd(collateral, debtToCover);
        // Give a 10% bonus to the liquidator
    ...
    /**
     * @dev Low-level internal function. do not call unless the function calling checks the health factor
     */
    ...
    /**
     * @notice returns the total amount of collateral deposited by the user, and the total value of that collateral in USD
     ...
     *  If the health factor is below 1, the user is insolvent and can be liquidated
     *  @param user The address of the user
     */
